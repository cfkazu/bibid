...
from .models import TwitterUser, CustomUser

# TODO: ユーザの自己紹介欄をどこかに作る:Email
# TODO:ユーザページを作る.ユーザページをもとに、投稿されている画像を新しい順に何個か表示する
...
def create_update_user_from_twitter(twitter_user_new):
    ...
    if twitter_user is None:
        user = CustomUser.objects.filter(username=twitter_user_new.screen_name).first()
        if user is None:
            user = CustomUser(username=twitter_user_new.screen_name,
                              first_name=twitter_user_new.name, profile_url=twitter_user_new.profile_image_url)
            user.save()
        print("image_url")
        ...
        user.profile_url = twitter_user_new.profile_image_url
        user.save()
